...
extension UserManager {
    ...
    func fetchCurrentUser() async throws -> User {
        try await documentReference(Auth.auth().currentUser?.uid ?? "").getDocument(as: User.self)
    }
    ...
    func updateBookmark(
        email: String,
        provider: String,
        bookmarkedBookstores: [String]
    ) async throws {
        let querySnapshot = try await collectionReference
            .whereField("email", isEqualTo: email)
            .whereField("provider", isEqualTo: provider)
            .getDocuments()
...
extension UserManager {
    ...
    func isExistingNickname(_ nickName: String) async throws -> Bool {
        let nickNames = try await collectionReference
            .getDocuments()
            .documents
            .map { $0.data() }
            .filter { String(describing: $0["nickName"] ?? "") == nickName }
...
extension UserManager {
    ...
    func fetchMyCurations(userID: String) async throws -> [Curation] {
        let querySnapShot = try await db
            .collection(CollectionPath.curations)
    ...
    func fetchCommentedCurations(userID: String) async throws -> [Curation] {
        ...
        for index in user.commentedCurations.indices {
            let index = (user.commentedCurations.count - 1 - index)
            // 최신 댓글 순서로 append
...
extension UserManager {
    ...
    func deleteAllCommentedCuration(curationID: String) async throws {
        let querySanpshot = try await db
            .collection(CollectionPath.curations)
            .document(curationID)
            .collection(CollectionPath.comments)
            .getDocuments()
        ...
        for index in comments.indices {
            let currentUserID = comments[index].userID

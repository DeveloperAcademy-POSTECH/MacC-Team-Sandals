...
final class HomeViewController: UIViewController {
    ...
    private func configureNavBarButtonItems() {
        let logoImage = UIImage(named: "KindyLogo")?
            .resizeImage(size: CGSize(width: 80, height: 20))
    ...
    @objc private func searchButtonTapped() {
        ...
        homeSearchViewController.setupData(
            items: model.bookstores,
            itemType: .bookstoreType,
            kinditorOfCuration: [:]
        )
        ...
        bookstoresTask = Task {
            ...
            if let bookstores = try? await BookstoreRequest().fetch() {
                ...
                case .authorizedWhenInUse, .authorizedAlways:
                    model.nearbyBookstores = sortBookstoresByMyLocation(
                        bookstores,
                        showOnlyThreeItems: true
                    )
